...
def inject_noisy_correspondence(dataset, noisy_rate, noisy_file =None):
    ...
    for i in range(nums):
        ...
        # pid, real_pid, image_id, image_path, text, cap_real_pid
        tmp = (pids[i],image_ids[i],images[i],captions[noisy_inx[i]], pids[noisy_inx[i]])
        dataset[i] = tmp
...
def rebuild_dataset(dataset):
    ...
    pid_cap = {}
    pid_img = {}
    for pid, image_id, img_path, caption, cap_r_pid in dataset:
        if pid not in pid_cap:
            pid_cap[pid] = []
        if pid not in pid_img:
            pid_img[pid] = []
        pid_cap[pid].append((caption, cap_r_pid))
        if img_path not in pid_img[pid]:
            pid_img[pid].append((img_path, image_id))
    # image_id = 0      
    for pid, img_path_list in pid_img.items():
        for img_path in img_path_list:
            for cap in pid_cap[pid]:
                new_dataset.append((pid, img_path[1], img_path[0], cap[0], cap[1]))
            # print(image_id)
            # image_id += 1
...
class ImageTextDataset(Dataset):
    def __init__(self,
        ...
        # self.dataset = rebuild_dataset(self.dataset)
        self.tokenizer = SimpleTokenizer()
    ...
    def __getitem__(self, index):
        pid, image_id, img_path, caption, cap_r_pid = self.dataset[index]
        img = read_image(img_path)
